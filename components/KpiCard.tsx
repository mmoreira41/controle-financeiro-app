...
export default function KpiCard({label, value, icon, onClick}:{label:string; value:number; icon?:string; onClick?:()=>void}) {
  ...
  return (
    <Component 
        onClick={onClick} 
        aria-label={label}
        className={`w-full text-left rounded-2xl bg-gray-800 p-4 ${onClick ? 'hover:bg-gray-700' : ''} transition-colors group focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 focus:ring-green-500`}
    >

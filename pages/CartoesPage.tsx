...
const CartoesPage: React.FC<CartoesPageProps> = ({
  ...
  const parcelasDoMes = useMemo(() => {
    const cartoesVisiveisIds = selectedView === 'all' 
      ? new Set(cartoes.map(c => c.id)) 
      : new Set([selectedView]);
    ...
    return (
        <div className="bg-gray-800 p-4 rounded-lg">
            <div className="flex justify-between items-center">
                ...
                <div className="flex items-center space-x-2">
                    {title === 'Fatura Atual' && (
                        <button onClick={() => openModal('pagar-fatura')} disabled={fatura.status === 'Paga' || selectedView === 'all'} className="bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-3 rounded-lg text-sm flex items-center space-x-1 disabled:bg-gray-600 disabled:cursor-not-allowed"> <DollarSign size={14}/><span>Pagar</span> </button>
                    )}
  ...
  return (
    <div className="animate-fade-in flex flex-col h-full">
        ...
        <div className="text-center">
            <h2 className="text-3xl font-bold text-white mb-2">Cartões de Crédito</h2>
        </div>
        ...
        <div className="flex flex-1 space-x-6 overflow-hidden">
            ...
            <div className="w-64 bg-gray-800 rounded-lg flex flex-col p-3">
                <div className="flex-grow space-y-2 overflow-y-auto no-scrollbar">
                    <button onClick={() => setSelectedView('all')} className={`w-full text-left p-3 rounded-lg flex items-center space-x-3 transition-colors ${selectedView === 'all' ? 'bg-green-500/80 text-white' : 'bg-gray-700/50 hover:bg-gray-700'}`}>
                        <Wallet size={20} /><span>Todos os Cartões</span>
                    </button>
                    <hr className="border-gray-700" />
                    {cartoes.sort((a,b) => a.apelido.localeCompare(b.apelido)).map(cartao => (
                        <button key={cartao.id} onClick={() => setSelectedView(cartao.id)} className={`w-full text-left p-3 rounded-lg flex items-center space-x-3 transition-colors relative overflow-hidden ${selectedView === cartao.id ? 'bg-green-500/80 text-white' : 'bg-gray-700/50 hover:bg-gray-700'}`}>
                            <div className="absolute left-0 top-0 h-full w-1.5" style={{ backgroundColor: cartao.cor || '#6b7280' }}></div>
                            <CreditCard size={20} className="ml-2"/> <span>{cartao.apelido}</span>
                        </button>
                    ))}
                ...
                <div className="pt-3 mt-auto border-t border-gray-700/50">
                    <button onClick={() => openModal('novo-cartao')} className="w-full text-center p-2 rounded-lg flex items-center justify-center space-x-2 transition-colors bg-gray-700/50 text-gray-300 hover:bg-gray-700 hover:text-white opacity-70 hover:opacity-100">
            ...
            <div className="flex-1 flex flex-col">
                ...
                <div className="bg-gray-800 rounded-lg p-4 flex-grow flex flex-col">
                    ...
                    <div className="flex space-x-2">
                        ...
                        {selectedCartao && (
                            <>
                                <button onClick={() => openModal('editar-cartao', { cartao: selectedCartao })} className="bg-gray-600 hover:bg-gray-500 text-white p-2 rounded-lg"><Pencil size={16} /></button>
                                <button onClick={() => deleteCartao(selectedCartao.id)} disabled={compras.some(c => c.cartao_id === selectedCartao.id)} className="bg-gray-600 hover:bg-red-500 text-white p-2 rounded-lg disabled:bg-gray-700 disabled:text-gray-500 disabled:cursor-not-allowed"> <Trash2 size={16} /> </button>
                            </>
                        )}
                    </div>
                    </div>
                        ...
                        <table className="w-full text-center text-sm">
                            <thead className="sticky top-0 bg-gray-800">
                                <tr>
                                    ...
                                    {selectedView === 'all' && <th className="p-3 font-semibold">Cartão</th>}
                                    <th className="p-3 font-semibold">Descrição</th>
                                    <th className="p-3 font-semibold">Categoria</th>
                                    <th className="p-3 font-semibold">Valor</th>
                                    <th className="p-3 font-semibold">Parcelas</th>
                                        ...
                                        {selectedView === 'all' && (
                                            <td className="p-3">
                                                <div className="flex justify-center items-center space-x-2">
                                                    <div className="w-1.5 h-4 rounded-full" style={{ backgroundColor: cartaoDaCompra?.cor || '#6b7280' }}></div>
                                                    <span className="whitespace-nowrap">{cartaoDaCompra?.apelido || 'N/A'}</span>
                                                </div>
                                            </td>
                                        )}
                            ...
                            {parcelasDoMes.length === 0 && ( <tr><td colSpan={selectedView === 'all' ? 7 : 6} className="text-center text-gray-400 py-8">Nenhuma parcela na fatura deste mês.</td></tr> )}
                            </tbody>
                        </table>
                    </div>
                </div>
